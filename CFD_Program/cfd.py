...
# Tighten bottom since we removed the PNG widgets
plt.subplots_adjust(left=0.07, right=0.98, top=0.90, bottom=0.24)
...
# -----------------------------
# Widgets (PNG import removed)
# -----------------------------
slider_ax = fig.add_axes([0.12, 0.19, 0.55, 0.04])
vel_slider = Slider(slider_ax, "Velocity", VEL_MIN, VEL_MAX, valinit=VEL_INIT)

mode_ax = fig.add_axes([0.83, 0.18, 0.14, 0.10])
mode_radio = RadioButtons(mode_ax, ("Analytic Cylinder", "Numerical Mask"), active=0)

holes_ax = fig.add_axes([0.12, 0.13, 0.12, 0.05])
holes_btn = Button(holes_ax, "Fix Holes")

copy_ax  = fig.add_axes([0.26, 0.13, 0.10, 0.05])
paste_ax = fig.add_axes([0.37, 0.13, 0.10, 0.05])
clear_ax = fig.add_axes([0.48, 0.13, 0.10, 0.05])
save_ax  = fig.add_axes([0.59, 0.13, 0.10, 0.05])
...
# -----------------------------
# Drawing tools — pause while drawing, recompute on finish
# -----------------------------
is_drawing = False
erase_mode = False
shift_held = False
...
def on_draw_toggled(_):
    """Pause simulation when drawing; on exit, recompute and resume."""
    global pending_draw_changes
    enabled = draw_check.get_status()[0]
    ax.set_navigate(not enabled)
    if enabled:
        timer.stop()            # pause updates
        info_txt.set_text("Draw Mode: ON (left-click add, right-click/Shift erase)")
    else:
        info_txt.set_text("")
        if pending_draw_changes:
            set_numerical_mode()
            restart_sim()       # recompute field + reset particles
...
def on_mouse_release(event):
    ...
    if not is_drawing:
        return
    is_drawing = False
    # Immediate recalibration for fast feedback (arrows + streamlines)
    set_numerical_mode()
    recompute()
